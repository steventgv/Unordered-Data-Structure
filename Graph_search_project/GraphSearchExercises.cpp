...
void GridGraph::removePoint(const IntPair& p1) {
    ...
    /*
    graph.removeEdge(IntPair(1, 2), IntPair(2, 2));
    graph.removeEdge(IntPair(2, 1), IntPair(2, 2));
    */

    /*
    for (const auto& x: originalNeighbors)
    {
       std::cout << x.first << std::endl;         
    }
    */

    // removeEdge(p1, IntPair(1, 2));
    // removeEdge(p1, IntPair(2, 1));

    for (const auto& x : originalNeighbors)
    {
       // std::cout << x.first << std::endl;
        removeEdge(p1, IntPair(x.first,x.second));
    }
...
std::list<PuzzleState> puzzleBFS(const PuzzleState& start, const PuzzleState& goal) {
    ...
    while (!exploreQ.empty() && !foundGoal && !tooManySteps) {
      ...
      //  GridGraph::NeighborSet neighbors = graph.adjacencyMap.at(curPoint);
        ...
        // std::array<int, 9>a= curState.getData();
